...
export default function DiscoveryProfilePanel() {
  ...
  React.useEffect(() => {
    apiFetch("/api/settings/discovery-profile")
      ...
      .then((row) => {
        ...
        setFinancing(Array.isArray(row.financingOptions) ? row.financingOptions.join(", ") : "");
        if (Array.isArray(row.testimonials)) {
          const txt = row.testimonials.map((t: Testimonial) => `${t.author}: ${t.quote}`).join("\n");
          setTestimonials(txt);
        }
      })
      .catch(() => {/* noop */});
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const fin = financing.split(",").map(s => s.trim()).filter(Boolean);
      const tms = testimonials
        ...
        .map(line => {
          ...
          return idx > 0
            ? { author: line.slice(0, idx).trim(), quote: line.slice(idx + 1).trim() }
        ...
        <div>
          <div className="grid-2">
            ...
            <div>
              ...
              <input className="form-input" value={services} onChange={e=>setServices(e.target.value)} />

              <label className="form-label">Financing options (comma-separated)</label>
              <input className="form-input" value={financing} onChange={e=>setFinancing(e.target.value)} />

              <label className="form-label">Testimonials (one per line as “Name: Quote”)</label>
              <textarea className="form-input" rows={8} value={testimonials} onChange={e=>setTestimonials(e.target.value)} />
            </div>
          </div>
